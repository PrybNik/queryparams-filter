...
We also need a ‘reset’ button that when clicked will remove the filters by clearing the query params from the url.
...
A summary of what the page should do is as follows:
1. Load the page.
2. On load, get the data from the “backend” (in this case, just mock the response)
3. save the data into state
4. Parse the query parameters from the URL
5. Filter the data by the query parameters
6. Render the data and filters
7. Add a `reset` button that removes the filters and modifies the URL to be /
